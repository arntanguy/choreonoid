...
void cnoid::computePointCloudNormals(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud,
...
SgMesh *cnoid::createSurfaceMesh(SgPointSet *pointSet)
{
  ...
  mesh->setVertices(pointSet->vertices());

  pcl::PointCloud<pcl::PointNormal>::Ptr cloud_with_normals(new pcl::PointCloud<pcl::PointNormal>
      (pointSet->vertices()->size(), 1));

  computePointCloudNormals(cloud, cloud_with_normals);
...
icp::IcpResults cnoid::alignPointCloudPointToPoint
(SgPointSet *target, SgPointSet *source, Affine3 &io_T, double maxCorrespondenceDistance, int maxIterations,
 double maxVariation)
{
  typedef pcl::PointCloud<pcl::PointXYZ> PointCloud;
  icp::IcpResults results;
  if (!target->hasVertices() || !source->hasVertices()) {
  ...
  results = icp.getResults();
  io_T = results.transformation.cast<Affine3::Scalar>();

  return results;
}

icp::IcpResults cnoid::alignPointCloudPointToPointSim3
(SgPointSet *target, SgPointSet *source, Affine3 &io_T, double maxCorrespondenceDistance, int maxIterations,
 double maxVariation)
{
...
icp::IcpResults cnoid::alignPointCloudPointToPlane
(SgPointSet *target, SgPointSet *source, Affine3 &io_T, double maxCorrespondenceDistance, int maxIterations,
 double maxVariation)
{
  ...
  PointCloudNormal::Ptr sourceCloudNormal(new PointCloudNormal(sourceCloud->size(), 1));
  PointCloudNormal::Ptr targetCloudNormal(new PointCloudNormal(targetCloud->size(), 1));

  computePointCloudNormals(sourceCloud, sourceCloudNormal);
  computePointCloudNormals(targetCloud, targetCloudNormal);
  ...
  icp.setParameters(param);
  icp.setInputCurrent(sourceCloudNormal);
  icp.setInputReference(targetCloudNormal);
